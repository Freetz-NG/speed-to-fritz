...
for file_n in $DIRI; do
    ...
    grep -q "uiDoFeedbackPage" "$file_n" || echo2 "  removed FeedbackPage setting from file:${file_n##*/}"
    sed -i -e 's|<? if neq $var:Annex A|<? if neq 2 2|' "$file_n"
    grep -q "<? if neq 2 2" "$file_n" && echo2 "  enable SNR or ANNEX setting on file:${file_n##*/}"
