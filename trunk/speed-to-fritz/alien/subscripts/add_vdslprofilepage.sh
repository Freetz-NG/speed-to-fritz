...
#  for file_n in /html/de/home/feedback.js /html/de/home/feedback.html ; do
#    [ -f "${SRC}/usr/www/${OEMLINK}/$file_n" ] && rm -f "${SRC}/usr/www/${OEMLINK}/$file_n" && echo2 "  Removed file: $file_n"
#  done
  for file_n in atm.html adsl.html bits.html overview.html; do
  sed -i -e '/uiDoSNRPage()/a\
<li><a href="javascript:DoVdslPorfilePage()">Vdsl Profile</a></li>'  "${SRC}/usr/www/${OEMLINK}/html/de/internet/$file_n"
  done 

cat >> "${SRC}/usr/www/${OEMLINK}/html/de//internet/awatch.js" << EOF
function uiDoVdslPorfilePage() {
