...
Section -CreateConfigFile
  ...
  FileWrite $0 "# This is a default config generated by the andLinux installer.$\r$\n"
  FileWrite $0 "# For more inFORMATIERENion, please see these sources:$\r$\n"
  FileWrite $0 "#   example.conf$\r$\n"
  FileWrite $0 "#   colinux-daemon.txt$\r$\n"
  FileWrite $0 "#   http://www.colinux.org/$\r$\n"
  FileWrite $0 "#   http://docs.andlinux.uclinux.org/doku.php?id=colinux$\r$\n"
  FileWrite $0 "$\r$\n"
  FileWrite $0 "# Please do not change unless you understand these settings$\r$\n"
  FileWrite $0 "kernel=vmlinux$\r$\n"
  FileWrite $0 "#base.drv is the complete compressed unbuntu or any other linux$\r$\n"
  ...
  FileWrite $0 "# This optin needs Win-Pcap installed, in your Windows network nebourhoud you can then bridge TAP with any Pysical Card.$\r$\n"
  FileWrite $0 "# Ubuntu must be updated as well, then: apt-get install pump, that eth0 is usabe with dhcp, ping via LAN possible, but not via WLAN$\r$\n"
